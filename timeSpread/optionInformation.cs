...
namespace timeSpread
{
    ...
    class OptionInformation
    {
        ...
        {
            //定义交易日期的类。
            TradeDay myTradeDay = new TradeDay(startDate, endDate);
            ...
            w.start();
            foreach (int today in myTradeDay.myTradeDay)
            {
                if (TradeDay.IsOptionExerciseDate(today) || today == myTradeDay.myTradeDay[myTradeDay.myTradeDay.Count - 1])
                {
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        option.startDate = TradeDay.DateTimeToInt((DateTime)optionList[i * 13 + 10 - 1]);
                        option.endDate = TradeDay.DateTimeToInt((DateTime)optionList[i * 13 + 11 - 1]);
